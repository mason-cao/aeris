...
## Corpus Check
- 118 files · ~94,766 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2142 nodes · 5382 edges · 110 communities (100 shown, 10 thin omitted)
- Extraction: 66% EXTRACTED · 34% INFERRED · 0% AMBIGUOUS · INFERRED: 1828 edges (avg confidence: 0.57)
...
### Community 1 - "L1"
Cohesion: 0.14
Nodes (40): ConsensusAnomaly, A merged anomaly record across detectors at a single (timestamp, location)., DetectionEngine, Detection orchestrator.  Runs the configured detectors (Z-score, STL, IsolationF, Orchestrates one or more detectors over a single time-series.      Detectors are, Standard 3-detector engine with each detector's library defaults., Run all configured detectors on ``series`` and merge via consensus.          ``s, IsolationForestDetector (+32 more)

### Community 2 - "Enrichment"
Cohesion: 0.22
Nodes (9): Anomaly, Claim, Explanation, _make_anomaly(), _make_claim(), _make_explanation(), TestClaimModel, TestExpertLabelModel (+1 more)

### Community 3 - "L106"
Cohesion: 0.09
Nodes (29): _amain(), archive_key(), available_strategies(), _days_between(), db_location_ids(), _format_result(), _gfs_cycles_in_range(), _iso_z() (+21 more)
...
### Community 5 - "L1"
Cohesion: 0.19
Nodes (9): MonkeyPatch, Any, Sentinel5PCollector, collector(), make_payload(), make_record(), TestSentinel5PColumnNormalize, TestSentinel5PFetch (+1 more)

### Community 6 - "Rate limiter"
Cohesion: 0.15
Nodes (34): BackfillResult, BackfillStrategy, NOAAGFSBackfill, OpenWeatherBackfill, Per-source historical loader interface., Walk the Sentinel-5P catalog backward in 48h windows, extracting columns.      C, Walk past GFS cycles within the NOMADS retention window., Documented no-op: the free OpenWeather tier has no historical API. (+26 more)

### Community 7 - "L92"
Cohesion: 0.13
Nodes (11): area_polygon(), extract_product_code(), granule_download_url(), ids_with_stored_columns(), _load_granule_arrays(), odata_filter(), parse_iso_datetime(), Product ids that already have a ``*_column`` DataPoint row.      Checked before (+3 more)
...
### Community 10 - "L1"
Cohesion: 0.10
Nodes (36): _amain(), _format_explanation(), generate_explanation(), main(), make_client(), _metric_points(), _parse_args(), persist_explanation() (+28 more)

### Community 11 - "L178"
Cohesion: 0.11
Nodes (22): CollectorClass, parse_archive_csv(), Map archive CSV rows to normalized DataPoints.      Columns: location_id, sensor, DataPointCreate, Normalized data point schema. All collectors output this format., within_target_radius(), location_within_target_radius(), normalize_openaq_unit() (+14 more)
...
### Community 16 - "L1"
Cohesion: 0.16
Nodes (21): _amain(), _cell_exists(), _format_summaries(), load_anomaly_set(), main(), ModelSweepSummary, _parse_args(), Sweep the eval anomaly set across all comparison models.  One cell = one (anomal (+13 more)
...
### Community 18 - "L46"
Cohesion: 0.12
Nodes (23): build_aux_inputs(), group_points_by_series(), GroupKey, _load_gfs_wind_cells(), Identifies one time-series: a single station / granule / grid cell., Group DataPoints into (source, metric, source_entity_id) time-series.      Drops, Assemble IsolationForest inputs by joining wind speed + pbl_height aux.      Win, Derive 10-m wind speed per grid cell, nearest cell to the station first.      GF (+15 more)

### Community 19 - "L1"
Cohesion: 0.12
Nodes (30): _amain(), _ask_verdict(), collect_claim_groups(), main(), _parse_args(), Interactive labeling tool for the expert evaluation set.  Loads one anomaly's co, Unique claim texts for an anomaly, in (model, step) first-seen order., One verdict letter, re-prompting until valid; None means quit. (+22 more)
...
### Community 21 - "L70"
Cohesion: 0.09
Nodes (50): Claim, Explanation, GroundingResult, LLMClient, Raw model output plus usage metadata from a single model call., Abstract base class for all LLM clients (local and cloud).      Subclasses imple, RawCompletion, _claim_row() (+42 more)
...
Cohesion: 0.12
Nodes (15): _measurement_timestamp(), OpenAQArchiveBackfill, OpenAQBackfill, Paginated historical fetch of OpenAQ sensor measurements., Bulk history from the OpenAQ S3 data archive instead of the API., Pull the measurement timestamp from an OpenAQ /measurements record.      The v3, Any, AsyncClient (+7 more)

### Community 24 - "L1"
Cohesion: 0.18
Nodes (10): CollectionResult, Result of a single collection run., attribute_value(), Column-product records minus those already extracted to the DB., Stream a granule to ``fd``, following redirects by hand.          The OData ``$v, safe_float(), Any, AsyncClient (+2 more)

### Community 25 - "L49"
Cohesion: 0.05
Nodes (50): _amain(), build_cross_source_summary(), context_window(), enrich_anomaly(), enrich_pending_anomalies(), _ensure_utc(), _Entry, _format_summary() (+42 more)
...
### Community 27 - "L1"
Cohesion: 0.16
Nodes (11): BaseModel, Call the model once and return raw text plus token counts.          ``schema`` i, Generate structured output validated against schema, retrying on parse failure., RawCompletion, _Attribution, _Attribution, _Attribution, _completion() (+3 more)
...
### Community 33 - "L1"
Cohesion: 0.14
Nodes (20): distance_km(), Great-circle distance between two WGS84 coordinates., _amain(), _ensure_utc(), _format_summary(), _load_aux_cells(), _load_gfs_component(), load_points() (+12 more)
...
### Community 41 - "L90"
Cohesion: 0.19
Nodes (4): Minimal stand-in for an httpx streaming response.      httpx rewrites Content-Le, _StubStreamClient, _StubStreamResponse, TestDownloadGranule

### Community 42 - "L1"
Cohesion: 0.19
Nodes (7): lifespan(), get_session(), FastAPI, health_check(), HealthResponse, AsyncSession, AsyncSession
...
### Community 44 - "L134"
Cohesion: 0.13
Nodes (20): BaseModel, RawCompletion, UUID, _anomaly(), ExplodingClient, _n_explanations(), Eval harness: sweep (anomaly x model) cells, idempotently and fault-isolated.  E, LLMClient returning scripted texts, one per call, counting calls. (+12 more)
...
### Community 46 - "L22"
Cohesion: 0.24
Nodes (5): _hourly_series(), _iso_inputs(), _run_kwargs(), _seasonal(), timedelta
...
### Community 50 - "L46"
Cohesion: 0.30
Nodes (11): DataSource, DataPointResponse, DataSourceResponse, fetch_data_sources(), get_data_by_source(), list_data_sources(), list_data_sources_legacy(), PaginatedDataPoints (+3 more)

### Community 51 - "L1"
Cohesion: 0.32
Nodes (9): _build_client(), _fast_limiter(), _location(), _measurement(), Catalog-only collector that records each window's ``$filter`` string., Mirror OpenAQ v3's actual /measurements response shape.      The timestamp lives, _recording_s5p_collector(), _sensor() (+1 more)

### Community 52 - "L20"
Cohesion: 0.05
Nodes (34): ArgumentParser, configure_logging(), _ExtraFormatter, Process-level logging for the collector entry points.  The scheduled runs redire, Append ``extra={...}`` fields as key=value pairs.      The collectors put their, collector_names(), create_collector(), create_collectors() (+26 more)
...
### Community 57 - "L15"
Cohesion: 0.08
Nodes (34): Anomaly, Base, EnrichmentRecord, ExpertLabel, DeclarativeBase, anomaly_bounding_box(), EnrichmentConfig, EnrichmentLine (+26 more)
...
Cohesion: 0.07
Nodes (26): ABC, BaseCollector, BaseCollector, Bulk insert normalized data points, ignoring duplicate observations., Update the DataSource record with collection status., Abstract base class for all data source collectors.      Subclasses must impleme, Fetch raw data from the external API., Transform raw API response into normalized DataPointCreate records. (+18 more)
...
### Community 73 - "L126"
Cohesion: 0.18
Nodes (6): BoundingBox, offset_coordinate(), Return min lon, min lat, max lon, max lat., Approximate coordinate offset for short distances., target_bounding_box(), TestExtractColumnsTokenRefresh
...
Cohesion: 0.24
Nodes (4): parse_cycle_time(), collector(), TestLoadCycle, TestParseCycleTime
...
- **Why does `DataPoint` connect `Data routes` to `L1`, `L106`, `L1`, `Rate limiter`, `L92`, `L1`, `L178`, `L46`, `Historical backfill`, `L1`, `L49`, `L296`, `L1`, `L41`, `L158`, `L90`, `L46`, `L1`, `L15`, `L14`, `L99`, `L126`, `Community 82`, `Community 99`?**
  _High betweenness centrality (0.229) - this node is a cross-community bridge._
- **Why does `Anomaly` connect `L15` to `L1`, `L1`, `Enrichment`, `L158`, `L1`, `L134`, `L46`, `L1`, `Community 83`, `L70`, `L99`, `L49`, `L296`?**
